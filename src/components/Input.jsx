...
const Input = ({ title, icon, value, legend, onChange, min, placeholder }) => {
  ...
  return (
    <section className='relative flex flex-col'>
      ...
      <input
        className={`bg-very-light-grayish-cyan h-12 text-very-dark-cyan font-bold text-2xl text-right px-5 rounded-[5px] placeholder:text-grayish-cyan border-2 border-transparent outline-none focus:border-strong-cyan ${error ? 'error' : ''}`}
        type="number"
        value={value || ''}
